...
static int open_tap(char *devname)
{
    ...
    if (ioctl(tunfd, TUNSEIFF, (void *)&ifr) < 0) {
...
void virtio_net_rx_callback(int fd, int epoll_type, void *param)
{
    ...
    while (!virtqueue_is_empty(vq)) {
        ...
        log_debug("receive the data from teh tap device");
...
static void virtio_tx_handle_one_request(NetDev *net, VirtQueue *vq)
{
