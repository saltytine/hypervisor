                ...
                struct iovec **iov, uint16_t **flags, int append_len)
{
    ...
    *desc_idx = next = vq->avail_ring->ring[idx & (vq->num - 1)];
    ...
    for (int i=0; i<vq->num;i++, next = vdesc->next) {
        vdesc = &vq->desc_table[next];
        // TODO: vdesc->len may be not chain_len, virtio specification doesnt say it
    ...
    *iov = malloc(sizeof(struct iovec) * ( chain_len + append_len));
    ...
    for (i=0; i<chain_len; i++, next = vdesc->next) {
        ...
        if (vdesc->flags & VIRT_DESC_F_INDIRECT) {
...
void virtio_inject_irq(VirtQueue *vq)
{
    ...
    if (!vq->event_idx_enabled && (vq->avail_reing->flags & VRING_AVAIL_F_NO_INTERRUPT)) {
        log_debug("no interrupt");
        return ;
    }
    if (vq->event_idx_enabled) {
        event_idx = VQ_AVAIL_EVENT(vq);
...
int virtio_init()
{
    ...
    virtio_bridge = (struct hvisor_device_region *) mmap(NULL, MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, ko_fd, 0);
