    ...
    PT::VA: Ord,
{
    ...
    /// It should only be used when accessing an address in el2, but hypervisor doesn't have the mapping
    pub fn map_temporary(
        ...
        if size > NUM_TEMPORARY_PAGES * PAGE_SIZE {
            warn!("Trying to map a too big space in the temporary area");
            return hv_result_err(EINVAL);
        }
